...
export const customFieldsRepo = {
  async findByBoardId(boardId: string): Promise<CustomField[]> {
    const db = getDb();
    const rows = await db.select().from(customFields).where(eq(customFields.boardId, boardId)).orderBy(customFields.position);
  ...
  async create(boardId: string, dto: CreateCustomFieldDto): Promise<CustomField> {
    const db = getDb();
    const rows = await db.insert(customFields).values({
      boardId,
      name: dto.name.trim(),
      type: dto.type,
  ...
  async update(id: string, dto: UpdateCustomFieldDto): Promise<CustomField | null> {
    ...
    const rows = await db.update(customFields).set(updates).where(eq(customFields.id, id)).returning();
    return rows[0] ? mapRow(rows[0]) : null;
  },
