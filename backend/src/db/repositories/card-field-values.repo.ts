...
export const cardFieldValuesRepo = {
  async findByCardId(cardId: string): Promise<CardFieldValue[]> {
    const db = getDb();
    const rows = await db.select().from(cardCustomFieldValues).where(eq(cardCustomFieldValues.cardId, cardId));
    return rows.map(mapRow);
  },

  async upsert(cardId: string, fieldId: string, dto: UpsertCardFieldValueDto): Promise<CardFieldValue> {
    const db = getDb();
    const rows = await db.insert(cardCustomFieldValues).values({
      cardId,
      fieldId,
      value: dto.value,
    }).onConflictDoUpdate({
  ...
  async remove(cardId: string, fieldId: string): Promise<boolean> {
    const db = getDb();
    const rows = await db.delete(cardCustomFieldValues).where(
      and(
        eq(cardCustomFieldValues.cardId, cardId),
        eq(cardCustomFieldValues.fieldId, fieldId),
      )
    ).returning();
